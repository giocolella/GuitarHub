function validateModifyProductForm(event) {
    ...
    let productNameRegex = /^[a-zA-Z0-9\s.,&'_-]*$/;
    if (!productNameRegex.test(productName)) {
      event.preventDefault();
      $('#errorDisplay').html("Il nome pu&ograve contenere solo lettere numeri ed alcuni caratteri speciali");
    return false;
    }
    
    let productDescriptionRegex = /^[a-zA-Z0-9\s.,&'_-]*$/;
    ...
    if (!productDescriptionRegex.test(productShortDescription)) {
      event.preventDefault();
      $('#errorDisplay').html("La descrizione breve pu&ograve contenere solo lettere numeri ed alcuni caratteri speciali");
    return false;
    }
    
    let productBrandRegex = /^[a-zA-Z0-9\s.,&'_-]*$/;
    if (!productBrandRegex.test(productBrand)) {
      event.preventDefault();
      $('#errorDisplay').html("La marca pu&ograve contenere solo lettere numeri ed alcuni caratteri speciali");
    return false;
    }
 ...
 function validateModifyProductPriceForm(event) {
  ...
  function validateModifyImagePathForm(event) {
   let productImagePath = document.getElementById("imagePath").value;
    ...
    $('#imagePath').css("border","2px solid green");
    $('#errorDisplay').html("");  
    return true;
  }
