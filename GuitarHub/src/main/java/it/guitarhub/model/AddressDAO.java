...
public class AddressDAO implements ImplementDAO<Address> {
	...
	public Address doRetriveByUser(int idUser) throws SQLException {
		String sql = "SELECT i.codice AS ic, i.nomeInd, i.cognomeInd, i.indirizzo, i.codPostale, i.city, i.provincia, i.numeroCivico, i.telefono "
				   + " FROM " + TABLE_NAME + " AS i WHERE i.idUser=?";
		Address addr = null;
		try (var conn = ds.getConnection()) {
			try (var stmt = conn.prepareStatement(sql)) {
