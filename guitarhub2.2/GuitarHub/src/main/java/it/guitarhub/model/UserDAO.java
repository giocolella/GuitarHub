...
public class UserDAO implements ImplementDAO<User> {
	...
	public void doSaveAddress(User user, Address address) throws SQLException {
		String sql = "INSERT INTO Indirizzo "
				   + " (idUser,nomeInd,cognomeInd,indirizzo,codPostale,city,provincia,telefono,preferito) "
				   + " VALUES (?,?,?,?,?,?,?,?,?)";
		
		if(address.isPreferred()==true) {
			String updateSQL= "UPDATE Indirizzo SET preferito=null WHERE idUser=?";
	...
	public void doSetPreferredAddress(User user, int id) throws SQLException {
		String sql1 = "UPDATE Indirizzo SET preferred = NULL WHERE idUser = ?";
		String sql2 = "UPDATE Indirizzo SET preferred = true WHERE codice = ? AND idUser = ?";
	...
	public void doSaveCompleted(User dao) throws SQLException {
		String sql = "UPDATE TABLE_NAME As u SET u.nome = ?, u.cognome = ?,u.dataNascita = ?,u.sesso = ?,u.cardNumber = ?,u.cardExpDate = ?,u.cvv = ? WHERE u.id = ?;";
		try (var conn = ds.getConnection()) {
			try (var stmt = conn.prepareStatement(sql)) {
				stmt.setString(1, dao.getFirstName());
				stmt.setString(2, dao.getLastName());
				stmt.setDate(3, java.sql.Date.valueOf(dao.getBirthday()));
				stmt.setString(4, dao.getGender());
				stmt.setString(5, dao.getCardNumber());
				stmt.setDate(6, java.sql.Date.valueOf(dao.getCardExpDate()));
				stmt.setString(7, dao.getCvv());
