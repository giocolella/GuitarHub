...
public class ReviewDAO implements ImplementDAO<Review>{
	...
	public boolean doVerifySecond(User user, Product product) throws SQLException {
	...
	public void doSave(Review rw) throws SQLException {
		var sql = "INSERT INTO " + TABLE_NAME + " (idProd,idUser,titolo,descrizione,punteggio,dataRec) VALUES (?,?,?,?,?,?)";
		try (var conn = ds.getConnection()) {
			try (var stmt = conn.prepareStatement(sql)) {
				...
				stmt.setDate(6, java.sql.Date.valueOf(rw.getReviewDate()));
				stmt.executeUpdate();
			}
		}
	}
